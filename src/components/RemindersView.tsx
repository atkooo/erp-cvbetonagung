...
export default function RemindersView({
  ...
  const handleMarkCompleted = async (id: string, refNum: string) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire(
          "Gagal",
          "Terjadi kesalahan saat memperbarui status pengingat.",
          "error",
        );
      }
    }
  ...
  const getReminderTypeLabel = (type: string) => {
    switch (type) {
      ...
      case "low_stock":
        return "Stok Minimum Tercapai";
      case "project_deadline":
        return "Deadline Pekerjaan Proyek";
      case "po_delivery":
        return "Pengiriman PO Jatuh Tempo";
      default:
        return type;
    }
  };
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredReminders.map((rem) => (
                  <tr key={rem.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-3.5 font-mono text-cyan-600 font-bold">
                      {rem.referenceNumber}
                    </td>
                    ...
                    <td className="p-3.5">
                      <StatusPill tone={getPriorityTone(rem.priority)}>
                        {rem.priority}
                      </StatusPill>
                    </td>
                    ...
                    <td className="p-3.5">
                      <StatusPill
                        tone={rem.status === "completed" ? "emerald" : "cyan"}
                      >
                        {rem.status}
                      </StatusPill>
                    </td>
                    <td className="p-3.5 pr-5 text-right">
                      {rem.status !== "completed" ? (
                        <button
                          onClick={() =>
                            handleMarkCompleted(rem.id, rem.referenceNumber)
                          }
