...
export default function DashboardView({
  ...
  const lowStockProducts = products.filter(
    (p) =>
      p.stock <= p.minStock || p.status === "Menipis" || p.status === "Habis",
  );
  ...
  // Calculate Best Sellers from Sales Orders
  ...
  const bestSellers = Object.entries(productSalesMap)
    ...
    .map(([name, count]) => {
      ...
      return {
        name,
        category: prod ? prod.category : "Umum",
        count,
        percentage: 0, // Will compute later if total known
      };
    });

  const totalBestSellerUnits = bestSellers.reduce(
    (acc, curr) => acc + curr.count,
    0,
  );
  bestSellers.forEach((bs) => {
    if (totalBestSellerUnits > 0) {
      bs.percentage = Math.round((bs.count / totalBestSellerUnits) * 100);
    }
  });
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
              <div>
                ...
                <button
                  ...
                  className="text-[10px] hover:underline text-slate-600 flex items-center gap-1 mt-2.5 font-bold"
                >
            ...
            >
              <div>
                ...
                <h3 className="text-xl font-bold font-sans text-red-600 mt-1">
                  {lowStockCount} SKU Menipis
                </h3>
                <button
                  onClick={() => onNavigate("stock-management")}
                  className="text-[10px] hover:underline text-red-700 flex items-center gap-1 mt-2.5 font-bold"
                >
                  Butuh Restock PO &rarr;
                </button>
              </div>
              <div
                className={`p-3 rounded-lg ${lowStockCount > 0 ? "bg-red-100 text-red-600" : "bg-slate-100 text-slate-400"}`}
              >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div
              ...
              className="p-3.5 bg-slate-50 hover:bg-slate-100 cursor-pointer rounded-xl border border-slate-200 transition-colors flex items-center gap-3"
            >
              <div className="p-2 bg-slate-200/60 rounded-lg text-slate-600">
                <Users size={16} />
              </div>
              <div>
                <p className="text-[9px] uppercase tracking-wider text-slate-400 font-mono font-bold">
                  Total Customer
                </p>
                <p className="text-sm font-bold text-slate-800">
                  {totalCustomers} Mandor/Takmir
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm font-bold text-slate-800">
                  {unpaidInvoices.length} Dokumen Tagihan
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
                <div>
                  <h3 className="font-sans font-bold text-sm text-slate-800">
                    Analisis Proyek & Penjualan Bulanan (Semester I 2026)
                  </h3>
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    Pendapatan kotor realisasi proyek custom dan beton pracetak
                  </p>
                </div>
              ...
              <div className="mt-5 pt-3 border-t border-slate-100">
                <button
                  onClick={() => {
                    ...
                    onTriggerNotification(
                      "Membuka Laporan Analisis Penjualan Lengkap",
                    );
                  }}
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-sans font-bold text-sm text-slate-800">
                    Alert Stok Menipis (&le; Batas Minimum)
                  </h3>
                </div>
                ...
                {lowStockProducts.length === 0 ? (
                  <div className="text-center py-8 text-slate-400">
                    ...
                    <p className="text-[10px]">
                      Semua produk memiliki kuantitas melebihi batas minimum
                      reorder.
                    </p>
                  </div>
                ) : (
                  lowStockProducts.map((p, idx) => (
                    <div
                      key={idx}
                      className="p-3 bg-rose-50/50 hover:bg-rose-50 rounded-xl border border-rose-100/60 flex items-center justify-between"
                    >
                      <div>
                      ...
                      <div className="flex items-center gap-3.5 shrink-0 pl-2">
                        ...
                        <button
                          onClick={() => {
                            onTriggerNotification(
                              `Mengirim Purchase Request (PO) otomatis untuk restock: ${p.sku}`,
                            );
                          }}
                  ...
                  filteredActiveProjects.map((proj) => {
                    ...
                    return (
                      <div
                        key={proj.id}
                        onClick={() =>
                          onNavigateToProject("project-detail", proj.id)
                        }
                      ...
                      >
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] text-slate-500">
                            Jenis: <strong>{proj.projectType}</strong> |
                            Spesifikasi: <strong>{proj.projectSpec}</strong> |
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
              <h3 className="font-sans font-bold text-sm text-slate-800">
                Aktivitas & Mutasi Material Terbaru
              </h3>
              <span className="text-[10px] text-slate-400 font-mono">
                Mei 25 - Mei 30
              </span>
                ...
                <div className="space-y-2">
                  {salesOrders.length === 0 ? (
                    ...
                    salesOrders.slice(0, 3).map((so, idx) => (
                      <div
                        key={idx}
                        className="p-2.5 bg-slate-50 rounded-lg border border-slate-200/40 text-xs flex items-center justify-between"
                      >
                        <div>
                        ...
                        >
                          ...
                          <div className="text-right">
                            <span className="font-bold text-rose-600 block">
                              {formatIDR(inv.total - inv.paidAmount)}
                            </span>
