...
export default function EmployeeLoanView({
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-xl leading-relaxed">
              Kelola pengajuan pinjaman/kasbon dan atur pemotongan otomatis per
              payroll.
            </p>
          </div>
          <button
            onClick={() => setIsModalOpen(true)}
            className="px-4 py-2 bg-slate-900 text-white rounded-xl font-bold flex items-center gap-2 shadow-sm hover:bg-slate-800 transition-colors"
          >
            <Plus size={14} />
          ...
          <tbody className="divide-y divide-slate-100">
            {loans.map((l) => (
              <tr key={l.id} className="hover:bg-slate-50/50">
                ...
                <td className="p-3.5 font-mono">
                  Rp {l.installment.toLocaleString("id-ID")}
                </td>
                <td className="p-3.5">
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-bold border ${l.status === "Approved" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}
                  >
                    {l.status}
                  </span>
                </td>
                <td className="p-3.5 pr-5 text-right">
                  {l.status === "Pending" && (
                    <div className="flex justify-end gap-1.5">
                      <button
                        onClick={() =>
                          onTriggerNotification("Pinjaman disetujui")
                        }
                      ...
                      <button
                        onClick={() =>
                          onTriggerNotification("Pinjaman ditolak")
                        }
                  ...
                  {l.status === "Approved" && (
                    <button className="px-2 py-1 bg-slate-100 hover:bg-slate-200 font-bold rounded text-slate-600">
                      Detail
                    </button>
                  )}
                </td>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl border border-slate-200 shadow-2xl p-5">
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={() => {
                  onTriggerNotification("Pengajuan kasbon berhasil disimpan");
                  setIsModalOpen(false);
                }}
                className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-2 rounded-lg mt-2 transition-colors"
              >
                Simpan & Ajukan
              </button>
            </div>
