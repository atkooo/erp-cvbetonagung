...
export default function RfqPicker({
  ...
  const selectedRfq = rfqs.find(rfq => rfq.rfqNumber === value || rfq.id === value);
      ...
      >
        <div className="flex items-center gap-2 overflow-hidden w-full">
          ...
          {isLoading && value && !selectedRfq ? (
            ...
            <span className={`text-xs truncate ${selectedRfq ? 'text-slate-800 font-bold font-mono' : 'text-slate-400'}`}>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[85vh]">
            ...
            <div className="p-4 border-b flex items-center justify-between bg-slate-50 rounded-t-xl">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-slate-800 text-sm">Pilih Referensi RFQ {statusFilter ? `(${statusFilter})` : ''}</h3>
                  <p className="text-[10px] text-slate-500">Pilih dari daftar RFQ yang sudah Diterima / Selesai</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-2 bg-slate-50">
              {isLoading ? (
                <div className="p-8 text-center text-slate-400 text-xs">Memuat data RFQ...</div>
              ) : filteredRfqs.length === 0 ? (
                <div className="p-8 text-center text-slate-400 text-xs">Data RFQ dengan status '{statusFilter}' tidak ditemukan.</div>
              ) : (
                  ...
                  {filteredRfqs.map(rfq => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-bold text-xs text-slate-800 font-mono">{rfq.rfqNumber}</div>
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded">{rfq.supplierName}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="text-right flex items-center gap-4">
                        <div className="text-right">
                          <div className="text-[10px] text-slate-500 mb-0.5">Berlaku Sampai</div>
