...
export default function WarehouseMasterView({
  onTriggerNotification,
}: WarehouseMasterViewProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      onTriggerNotification(
        error instanceof Error ? error.message : "Gagal memuat master gudang.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSaveWarehouse = async (e: React.FormEvent) => {
    ...
    const payload = {
      code:
        warehouseForm.code === "AUTO GENERATED"
          ? ""
          : warehouseForm.code.trim().toUpperCase(),
    ...
    try {
      if (warehouseForm.id) {
        await apiClient.put(
          `/master-data/warehouses/${warehouseForm.id}`,
          payload,
        );
        onTriggerNotification(`Gudang ${payload.name} diperbarui.`);
      } else {
        const response = await apiClient.post<{ data: WarehouseDto }>(
      ...
      Swal.fire(
        "Gagal",
        error instanceof Error ? error.message : "Gagal menyimpan gudang.",
        "error",
      );
    }
  };
  ...
  const handleSaveLocation = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      code:
        locationForm.code === "AUTO GENERATED"
          ? ""
          : locationForm.code.trim().toUpperCase(),
    ...
    try {
      if (locationForm.id) {
        await apiClient.put(
          `/master-data/storage-locations/${locationForm.id}`,
          payload,
        );
      ...
      Swal.fire(
        "Gagal",
        error instanceof Error ? error.message : "Gagal menyimpan lokasi/rak.",
        "error",
      );
    }
  };
  ...
  const handleDeleteWarehouse = async (warehouse: WarehouseDto) => {
    ...
    try {
      ...
      Swal.fire(
        "Gagal",
        error instanceof Error ? error.message : "Gudang tidak bisa dihapus.",
        "error",
      );
    }
  };
  ...
  const handleDeleteLocation = async (location: LocationDto) => {
    ...
    try {
      ...
      Swal.fire(
        "Gagal",
        error instanceof Error ? error.message : "Lokasi tidak bisa dihapus.",
        "error",
      );
    }
  };
    ...
    <div className="space-y-6 text-xs font-sans">
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">
              Kelola gudang, area penyimpanan, dan kode rak sebagai master data
              inventory.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={fetchData}
            className="px-3 py-2 border rounded-lg font-bold text-slate-600 hover:bg-slate-50 flex items-center gap-1.5"
          >
          ...
          <button
            ...
            className="px-3 py-2 bg-slate-900 text-white rounded-lg font-bold hover:bg-slate-800 flex items-center gap-1.5"
          >
            <Plus size={12} />
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-400 font-mono">
              {warehouses.length} data
            </span>
          </div>
          <div className="divide-y divide-slate-100 max-h-155 overflow-y-auto">
            {warehouses.length === 0 ? (
              ...
              warehouses.map((warehouse) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <span
                        className={`font-mono text-[10px] ${selectedWarehouseId === warehouse.id ? "text-slate-300" : "text-slate-400"}`}
                      >
                        {warehouse.code}
                      </span>
                      <p className="font-bold mt-0.5">{warehouse.name}</p>
                      <p
                        className={`text-[10px] mt-1 ${selectedWarehouseId === warehouse.id ? "text-slate-300" : "text-slate-500"}`}
                      >
                        {warehouse.address || "Alamat belum diisi"}
                      </p>
                    </div>
                    <span
                      className={`px-2 py-0.5 rounded border text-[10px] font-bold ${selectedWarehouseId === warehouse.id ? "border-slate-600 text-slate-200" : "border-slate-200 text-slate-500 bg-slate-50"}`}
                    >
          ...
          <div className="p-4 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              {selectedWarehouse && (
                <>
                  <button
                    onClick={() => openWarehouseEdit(selectedWarehouse)}
                    className="px-3 py-2 border rounded-lg font-bold text-slate-600 hover:bg-slate-50 flex items-center gap-1.5"
                  >
                    <Edit size={12} />
                  ...
                  <button
                    ...
                    className="px-3 py-2 border border-rose-200 rounded-lg font-bold text-rose-600 hover:bg-rose-50 flex items-center gap-1.5"
                  >
                    <Trash2 size={12} />
                    <span>Hapus</span>
                  </button>
              ...
              <button
                ...
                className="px-3 py-2 bg-slate-900 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg font-bold hover:bg-slate-800 flex items-center gap-1.5"
              >
                <Plus size={12} />
                      ...
                      <td className="p-3.5 pr-5 text-right">
                        <div className="flex justify-end gap-1.5">
                          <button
                            ...
                            className="px-2 py-1 border rounded text-[10px] font-bold text-slate-600 hover:bg-slate-50"
                          >
                            Edit
                          </button>
                          <button
                            onClick={() => handleDeleteLocation(location)}
                            className="px-2 py-1 border border-rose-200 rounded text-[10px] font-bold text-rose-600 hover:bg-rose-50"
                          >
                            Hapus
                          </button>
                        </div>
      ...
      {showWarehouseModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden border border-slate-200">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between">
              <h4 className="font-bold text-slate-800 text-[11px] uppercase tracking-wider">
                {warehouseForm.id ? "Edit Gudang" : "Tambah Gudang"}
              </h4>
              <button
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="space-y-1.5">
                  ...
                  <input
                    ...
                    readOnly
                    disabled
                    className="w-full px-3 py-2 border rounded-lg font-mono uppercase bg-slate-100 text-slate-500 cursor-not-allowed"
                  />
                </label>
                <label className="space-y-1.5">
      ...
      {showLocationModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden border border-slate-200">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between">
              <h4 className="font-bold text-slate-800 text-[11px] uppercase tracking-wider">
                {locationForm.id ? "Edit Rak" : "Tambah Rak"}
              </h4>
              <button
