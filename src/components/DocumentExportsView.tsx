...
export default function DocumentExportsView({
  onTriggerNotification,
}: DocumentExportsViewProps) {
  ...
  const spreadsheetCount = exports.filter(
    (e) => e.exportFormat === "XLSX" || e.exportFormat === "CSV",
  ).length;
              ...
              <tbody className="divide-y divide-slate-100">
                {exports.map((exp) => (
                  <tr key={exp.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-3.5">
                      <StatusPill
                        tone={exp.exportFormat === "PDF" ? "cyan" : "amber"}
                      >
                        {exp.exportFormat}
                      </StatusPill>
                    </td>
